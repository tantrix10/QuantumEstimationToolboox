...
def D(f, g, l, n, t=1):
    ...
    for i in range(len(f)):
        ms.append(qt.Qobj(Mm(f[i], g[i], l, n)))
    #print('M: ',M)
    
    #calculate the corresponding unitary
    #print(M)
    Mu = reduce(lambda x,y: x*y, [qt.Qobj.expm(-1j*(t/len(f))*m) for m in ms])
...
def test_many_fisher_infos():
    l = 1
    n = 4
    for i in range(10):
        f = [np.random.rand() for i in range(5)]
        g = [np.random.rand() for i in range(5)]
        fisher_info_linear = fisher_info_matrix(f,g,l,n)

        assert np.isclose(fisher_info_linear, fisher_info(f,g,l,n, exp.D_vec))
...
#     # print('exp space qfi     : ', fisher_info(fd,gd,l,n, exp.D) )
#     # print('lin stay in D qfi  :', fisher_info_matrix(fd,gd,l,n))
#     # outs = []
#     # for i in range(40):
#     #     f = [4-8*np.random.random() for i in range(15)]
#     #     g = [4-8*np.random.random() for i in range(15)]
#     #     outs.append(fisher_info_matrix(f,g,1,n))
#     # print(max(outs))
