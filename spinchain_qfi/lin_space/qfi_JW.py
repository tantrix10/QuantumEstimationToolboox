...
def bloch_vec( d, n):
    #print("D matrix: ",d)
    part1 = sum( [d[0][i]*d[1][i+n+1] for i in range((n+1)) ] )
    part2 = sum( [np.conj(d[0][i])*d[1][i] for i in range((n+1)) ] )
    e_x = np.real(-2*(part1+part2) )
    e_y = np.imag(-2*(part1+ part2 ))
    #d_fac = np.real(sum(d[0]))
    e_z =(2*sum( [np.abs(d[1][i])**2 for i in range((n+1)) ] )-1 )
    return np.array([e_x, e_y, e_z])

def bloch_vech_deriv(d, deriv,n):
    """
    ...
    """
    part1 = sum( [d[0][i]*deriv[1][i+n+1] for i in range((n+1)) ] )
    part2 = sum( [np.conj(d[0][i])*deriv[1][i] for i in range((n+1)) ] )
    part3 = sum( [deriv[0][i]*d[1][i+n+1] for i in range((n+1)) ] )
    part4 = sum( [np.conj(deriv[0][i])*d[1][i] for i in range((n+1)) ] )

    e_x = np.real(-2*(part1+part2+part3+ part4) )
    e_y = np.imag(-2*(part1+ part2 + part3+part4 ))
    e_z = 2*np.abs(sum( [(d[1][i]*np.conj(deriv[1][i]) + deriv[1][i]*np.conj(d[1][i]) ) for i in range((n+1)) ] ) )
    return np.array([e_x, e_y, e_z])
...
class TSlotCompSysId(tslotcomp.TSlotCompUpdateAll):
    ...
    def recompute_evolution(self):
        ...
        if prop_comp.grad_exact:
            for k in range(n_ts):  #  tslot index, inclusive
...
class FidCompQubitQFI(fidcomp.FidelityComputer):
    ...
    def reset(self):
        ...
        self.d_bloch = None
        self.mixed = False
        #self.n = int((self.parent.drift_dir()/2) -1 )
    ...
    def computeBloch(self):
        ...
        d1 = reduce( lambda x,y: x*y, dyn.prop).full()
        # print(d1)
        # d = dyn.prop[-1].full()
        # print(d)
        #d = dyn._fwd_evo[-1]
        #print(self.parent.dyn_shape)
        n = int((self.parent.dyn_shape[1]/2) -1 )
        self.bloch = bloch_vec(d1,n)

        self.evo_der = np.zeros([self.parent.get_drift_dim(), self.parent.get_drift_dim()])
        for k in range(n_ts):
            derk = dyn._prop_der[k].dot(dyn.prop[k])
            if k+1 < n_ts:
                derk = dyn._onwd_evo[k+1].dot(derk)
            self.evo_der = self.evo_der + derk
        self.evo_der = sum(dyn._prop_der[k].dot(dyn.prop[k]) for k in range(n_ts))
        # print("deriv mat: ", self.evo_der)
        # print("unitr mat: ", d)
        # TODO: fix this temp fix
        self.d_bloch = bloch_vech_deriv(d1, self.evo_der@d1.conj().transpose(), n)
        # print("bloch vec deriv: ",self.d_bloch)
        #print("bloch vec:       ",self.bloch)
    ...
    def computeBloch_dergrad(self, k0, j0):
        ...
        evo_grad = None
        derk = dyn._prop_grad[k0, j0].dot(dyn.prop[k0])
        if k0+1 < n_ts:
        ...
        for k in range(k0+1, n_ts):
            ...
            if k+1 < n_ts:
                derk = dyn._onwd_evo[k+1].dot(derk)
            dergrad = dergrad + derk
        derk = dyn._prop_hessian[k0, j0].dot(dyn.prop[k0])
        if k0+1 < n_ts:
            derk = dyn._onwd_evo[k0+1].dot(derk)
        dergrad = dergrad + derk
        ...
        bloch_grad = 2 * bloch_vec( evo_grad, n)
        bloch_dergrad  = 2 * (bloch_vec( dergrad, n) + bloch_vec( self.evo_der, n))#this line really needs checking  bloch_vec( self.evo_der.dot(evo_grad), self.n))
        # TODO: check above


        #print('bloch derv',bloch_vec( self.evo_der, n))
    ...
    def compute_fidelity(self):
        ...
        """
        db = self.d_bloch
        self.fisher_info = dot(db, db)
        self.fidelity = self.fid_norm_func(self.fisher_info)
        # TODO: Typically 1-self.fidelity. I can set the rescaling
        # to be 4*n**2 * t**2 though
